...
func (r *Runner) LoadScripts() string {
	...
	vm.Set(SetOSVar, func(call otto.FunctionCall) otto.Value {
		args := call.ArgumentList
		varName := strings.ToUpper(args[0].String())
		defaultValue := varName
		if len(args) > 1 {
			defaultValue = args[1].String()
		}
		err := os.Setenv(varName, defaultValue)
		if err != nil {
			utils.ErrorF("Error setting environment variable: %v", err)
		}
		return otto.Value{}
	})
